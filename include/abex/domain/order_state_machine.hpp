...
[[nodiscard]] inline bool stale_sequence(const Order& order,
                                         const ExecutionReport& report) noexcept {
    return order.last_sequence && report.sequence && *report.sequence <= *order.last_sequence;
}

[[nodiscard]] inline Decimal offset_for(const StringMap<Decimal>& offsets,
...
[[nodiscard]] inline bool is_historical_generation(const Order& order,
...
[[nodiscard]] inline OrderStatus derive_status(const Order& order,
                                               const ExecutionReport& report,
...
[[nodiscard]] inline bool amendment_confirmed(const Order& order,
                                              const ExecutionReport& report) {
...
public:
    [[nodiscard]] static ApplyResult apply(Order& order, const ExecutionReport& report) {
        ...
        const bool historical = detail::is_historical_generation(order, report);
        ...
        const auto fill_offset = detail::offset_for(order.exchange_fill_offsets,
                                                     report.exchange_order_id);
        const auto quote_offset = detail::offset_for(order.exchange_quote_offsets,
                                                      report.exchange_order_id);
        ...
        const bool stale = detail::stale_sequence(order, report);
        ...
        if (report.cumulative_quote) {
            ...
            if (aggregate_quote >= order.cumulative_quote)
                order.cumulative_quote = aggregate_quote;
        } else if (merged_filled > order.filled_quantity && report.last_fill_price) {
            order.cumulative_quote +=
                (merged_filled - order.filled_quantity) * *report.last_fill_price;
        }
        ...
        order.status = detail::derive_status(order, report, aggregate_filled, historical, stale);
        ...
        case PendingAction::Amend:
            clear_pending = !historical &&
                            (is_terminal(order.status) ||
                             detail::amendment_confirmed(order, report));
            break;
        ...
        const bool changed =
            old_status != order.status || old_filled != order.filled_quantity ||
            old_quote != order.cumulative_quote || exchange_id_will_change ||
            old_price != order.price || old_quantity != order.quantity ||
            old_pending != order.pending_action;
